...
def _build_commercial(data, content, lang, n):
    ...
    # Full description at the bottom — AI-improved from form input
    desc_paras = [p.strip() for p in content['narrative'].split('\n\n') if p.strip()]
    desc_html = ''.join(f'<p class="text" style="margin-bottom:2mm;">{p}</p>' for p in desc_paras)
