...
def _build_market(data, content, lang, n):
    ...
    if not comparables and pm2_prop > 0:
        ...
        _m2c = float(data.get('metros_construidos') or 70)
        _m2s = data.get('metros_construidos', '—')
        _p0 = '{:,}'.format(int(pm2_prop * _m2c)).replace(',', '.')
        _p1 = '{:,}'.format(int((pm2_prop + delta) * _m2c)).replace(',', '.')
        _p2 = '{:,}'.format(int((pm2_prop - delta) * _m2c)).replace(',', '.')
        comparables = [
            {'tipo': tipo_str, 'ubicacion': barrio, 'precio': f'{_p0} €', 'm2': _m2s, 'nota': 'Precio similar zona'},
            {'tipo': tipo_str, 'ubicacion': f'{barrio} — reformado', 'precio': f'{_p1} €', 'm2': _m2s, 'nota': '+5% vs este activo'},
            {'tipo': tipo_str, 'ubicacion': f'{ciudad} — zona adyacente', 'precio': f'{_p2} €', 'm2': _m2s, 'nota': 'Menor calidad de zona'},
        ]
