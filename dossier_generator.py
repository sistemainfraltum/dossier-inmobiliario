...
def _build_commercial(data, content, lang, n):
    ...
    lead = all_paras[0] if all_paras else ''
    ...
    # ── Página B: descripción completa (p2 + p3 + p4) ───────────────────────────
    desc_paras = all_paras[1:]  # todo excepto el lead
    desc_html = ''.join(f'<p class="text" style="margin-bottom:4mm;">{p}</p>' for p in desc_paras)
