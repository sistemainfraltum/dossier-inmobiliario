...
def _build_commercial(data, content, lang, n):
    ...
    # Cuadrícula: hasta 9 fotos (3 filas de 3) para llenar la página
    num_rooms = max(6, min(9, len(fotos) if fotos else 9))
    img_height = '34mm' if num_rooms > 6 else '40mm'
    rooms_html = ''.join(
...
def _build_location(data, content, lang, n):
    ...
    return _page(
        ...
        f'  </div>'
        f'</div>'
        f'<div class="vgallery" style="grid-template-columns:1fr 1fr 1fr; margin-top:0;">{zone_html}</div>'
        f'{_foot(sh_ey, ciudad)}'
        f'</div>',
        bg_key='location'
    )
