...
def _build_commercial(data, content, lang, n):
    ...
    # Build room grid — images only, no captions or descriptions under each photo
    num_rooms = max(3, min(6, len(fotos) if fotos else 6))
    rooms_html = ''.join(
        f'<div class="room-item">'
        f'<img src="{get_room_src(i)}" alt="" style="height:38mm;"/>'
        f'</div>'
    ...
    # Description below images — p3 + p4 from narrative (form description, professionally rewritten)
    all_paras = [p.strip() for p in content['narrative'].split('\n\n') if p.strip()]
    bottom_paras = all_paras[2:]  # p3 (improved form description) + p4 (features)
    bottom_html = ''.join(f'<p class="text" style="margin-bottom:2mm;">{p}</p>' for p in bottom_paras)

    return _page(
        f'<div class="inner">'
        f'{_sh(sh_ey, sh_tt, n)}'
        f'<p class="lead">{lead}</p>'
        f'<div class="note" style="margin-bottom:3mm;">'
        f'{chips_html}'
        f'{"&nbsp;&nbsp;&middot;&nbsp;&nbsp;" + estado_desc if estado_desc and chips else estado_desc}'
        f'</div>'
        f'<div class="room-grid">{rooms_html}</div>'
        f'<div class="divider"></div>'
        f'<div style="max-height:95mm; overflow:hidden;">{bottom_html}</div>'
        f'{_foot(sh_ey, ciudad)}'
        f'</div>',
        bg_key='commercial'
    )
