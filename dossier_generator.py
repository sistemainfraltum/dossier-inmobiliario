...
.card {
    position: relative;
    padding: 5mm;
    border: 1.5px solid rgba(36,52,71,0.20);
...
.card .text { color: #1e2f42; font-size: 9.5pt; font-weight: 700; }

.card.stone {
    background: linear-gradient(135deg, #efe7da, #fff8f2);
    border-color: rgba(155,118,56,0.25);
}
.card.sky {
    background: linear-gradient(135deg, #edf5f8, #dceef6);
    border-color: rgba(85,132,156,0.25);
}
.card.sky h4 { color: #1e3d52; }
.card.sky .text { color: #1e3d52; font-weight: 700; }
...
.kpi {
    ...
    padding: 4mm;
    background: #ffffff;
    border: 2px solid rgba(36,52,71,0.18);
    box-shadow: 0 2px 6px rgba(30,47,64,0.10);
}
.kpi.dark {
...
.vgallery { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 2.5mm; margin: 3mm 0; }
.vitem { border: 1px solid rgba(201,168,76,0.22); overflow: hidden; background: var(--warm); }
.vitem img { display: block; width: 100%; height: 22mm; object-fit: cover; }
.vcap {
    padding: 2mm 3mm;
    background: linear-gradient(135deg, var(--warm), var(--ivory));
    font-family: 'Inter', Arial, sans-serif;
...
def _ai(content, key, fallback=''):
    """Obtiene contenido generado por Claude con fallback al valor por defecto."""
    ai = content.get('ai_content') or {}
    val = ai.get(key, '')
...
def _build_cover(data, content, lang):
    ...
    # Tagline de Claude: value_proposition como subtítulo en portada
    ai_vp = _ai(content, 'value_proposition', '')
    vp_html = (f'<p style="font-family:Inter,Arial,sans-serif; font-size:9.5pt; font-weight:400; '
               f'color:rgba(255,252,245,0.82); letter-spacing:0.02em; margin-top:3mm; '
               f'max-width:130mm; line-height:1.55;">{ai_vp}</p>') if ai_vp else ''
...
def _build_summary(data, content, lang, n):
    ...
    lead  = _ai(content, 'executive_lead',  paras[0] if paras else '')
    body2 = _ai(content, 'executive_body',  paras[1] if len(paras) > 1 else '')
    ...
    kpis_html = ''.join(_kpi(v, l, False) for v, l, d in kpis)

    comm     = content['commercial']
    prop_val = _ai(content, 'value_proposition', comm.get('propuesta_valor', ''))
    perf_c   = _ai(content, 'buyer_profile',     comm.get('perfil_comprador', ''))
    tesis    = _ai(content, 'investment_thesis_short', content['conclusions']['texto'])
...
# ─── COMMERCIAL DESCRIPTION — 2 páginas: imágenes + descripción ───────────────

def _build_commercial(data, content, lang, n):
    """Returns a list of 2 HTML page strings: [images_page, description_page]."""
    es = lang == 'es'
    fotos = data.get('foto_paths', [])
    ciudad = _s(data.get('ciudad'), '')
    m2 = _s(data.get('metros_construidos'))
    ...
    if es:
        sh_ey_img  = 'El Inmueble'; sh_tt_img  = 'Cada espacio, presentado con criterio y detalle.'
        sh_ey_desc = 'Descripcion del Inmueble'; sh_tt_desc = 'Caracteristicas, calidades y detalles del activo.'
        estado_desc = {
            'nuevo': 'Inmueble a estrenar con acabados de primera calidad en todos sus espacios.',
            'excelente': 'Excelente estado de conservacion.',
        ...
        ocupar_note = (
            '<div class="note" style="margin-bottom:4mm;">Listo para ocupar de inmediato, sin necesidad de intervenci&oacute;n alguna.</div>'
            if estado in ('nuevo', 'excelente') else ''
        )
    else:
        sh_ey_img  = 'The Property'; sh_tt_img  = 'Every space, presented with criterion and detail.'
        sh_ey_desc = 'Property Description'; sh_tt_desc = 'Features, finishes and details of the asset.'
        estado_desc = {
            'nuevo': 'Brand new property with first-class finishes throughout all spaces.',
            'excelente': 'Excellent condition.',
        ...
        ocupar_note = (
            '<div class="note" style="margin-bottom:4mm;">Ready for immediate occupation, no works required.</div>'
            if estado in ('nuevo', 'excelente') else ''
        )
    ...
    # ── Página A: párrafo lead + cuadrícula grande de imágenes ──────────────────
    all_paras = [p.strip() for p in content['narrative'].split('\n\n') if p.strip()]
    lead = _ai(content, 'commercial_lead', all_paras[0] if all_paras else '')

    # Data chips
    chips = []
    if m2: chips.append(f"{m2} m²")
    if dorms and dorms != '0': chips.append(f"{dorms} {'dorm.' if es else 'bed.'}")
    if banos and banos != '0': chips.append(f"{banos} {'ba&ntilde;.' if es else 'bath.'}")
    chips_html = ' &nbsp;&middot;&nbsp; '.join(f'<strong>{c}</strong>' for c in chips)
    note_content = chips_html
    if estado_desc:
        note_content += ('&nbsp;&nbsp;&middot;&nbsp;&nbsp;' if chips else '') + estado_desc

    # Cuadrícula: hasta 12 fotos (4 filas de 3) para llenar la página
    num_rooms = max(6, min(12, len(fotos) if fotos else 12))
    img_height = '28mm' if num_rooms > 9 else ('32mm' if num_rooms > 6 else '40mm')
    rooms_html = ''.join(
        f'<div class="room-item">'
        f'<img src="{get_room_src(i)}" alt="" style="height:{img_height};width:100%;object-fit:cover;border-radius:2mm;display:block;"/>'
        f'</div>'
        for i in range(num_rooms)
    )
    cols = 3
    grid_style = (f'display:grid;grid-template-columns:repeat({cols},1fr);'
                  f'gap:3mm;margin-top:3mm;margin-bottom:3mm;')

    page_a = _page(
        f'<div class="inner">'
        f'{_sh(sh_ey_img, sh_tt_img, n)}'
        f'<p class="lead" style="margin-bottom:3mm;">{lead}</p>'
        f'<div class="note" style="margin-bottom:3mm;">{note_content}</div>'
        f'<div style="{grid_style}">{rooms_html}</div>'
        f'{_foot(sh_ey_img, ciudad)}'
        f'</div>',
        bg_key='commercial'
    )

    # ── Página B: descripción completa ───────────────────────────────────────────
    ai_desc     = _ai(content, 'commercial_description', '')
    ai_high     = _ai(content, 'commercial_highlights', '')
    if ai_desc:
        desc_paras_text = [ai_desc] + ([ai_high] if ai_high else []) + all_paras[2:]
    else:
        desc_paras_text = all_paras[1:]
    desc_html = ''.join(f'<p class="text" style="margin-bottom:4mm;">{p}</p>' for p in desc_paras_text)

    page_b = _page(
        f'<div class="inner">'
        f'{_sh(sh_ey_desc, sh_tt_desc, n + 1)}'
        f'{ocupar_note}'
        f'{desc_html}'
        f'{_foot(sh_ey_desc, ciudad)}'
        f'</div>',
        bg_key='commercial'
    )
...
def _build_location(data, content, lang, n):
    ...
    lead = _ai(content, 'location_intro', paras[0] if paras else '')
    ai_depth = _ai(content, 'location_depth', '')
    ai_why   = _ai(content, 'location_why_here', '')
    body_paras_raw = [p for p in [ai_depth, ai_why] if p] or paras[1:3]
    body = ''.join(f'<p class="text" style="margin-bottom:2.5mm;">{p}</p>' for p in body_paras_raw)
    ...
    # ── Tabla de lugares cercanos reales (OpenStreetMap) ────────────────────────
    nearby = content.get('nearby_places', [])

    # Fallback: si no hay datos OSM, usar servicios_cercanos del formulario
    if not nearby:
        import re as _re
        srv_raw = (data.get('servicios_cercanos') or '').strip()
        if srv_raw:
            items = [x.strip() for x in _re.split(r'[,\n;]+', srv_raw) if x.strip()]
            nearby = []
            for item in items[:14]:
                dm = _re.search(r'(\d+[\.,]?\d*\s*(?:m|km|min|minutos|metros))', item, _re.IGNORECASE)
                if dm:
                    name_str = item[:dm.start()].strip(' -–,aáàä').strip()
                    try:
                        raw = _re.sub(r'[^\d]', '', dm.group(1).split()[0])
                        dist_m = int(raw) * (1000 if 'km' in dm.group(1).lower() else 1)
                    except Exception:
                        dist_m = 0
                else:
                    name_str = item
                    dist_m = 0
                nearby.append({'name': name_str, 'cat_es': '', 'cat_en': '', 'dist': dist_m})

    # ── Mapa estático ────────────────────────────────────────────────────────────
    map_b64 = content.get('map_b64')
    map_html = (
        f'<div style="margin-bottom:3mm;border-radius:2mm;overflow:hidden;">'
        f'<img src="{map_b64}" style="width:100%;height:auto;display:block;" alt="mapa"/>'
        f'</div>'
    ) if map_b64 else ''

    if nearby:
        srv_lbl = 'Servicios y puntos de inter&eacute;s pr&oacute;ximos' if es else 'Nearby services &amp; points of interest'
        if es:
            th_place = 'Lugar'; th_cat = 'Tipo'; th_dist = 'Distancia'
        else:
            th_place = 'Place'; th_cat = 'Type'; th_dist = 'Distance'
        header_row = (
            f'<tr>'
            f'<th style="width:45%;">{th_place}</th>'
            f'<th style="width:30%;">{th_cat}</th>'
            f'<th style="width:25%;text-align:right;">{th_dist}</th>'
            f'</tr>'
        )
        rows_srv = []
        for p in nearby:
            cat_lbl = p['cat_es'] if es else p['cat_en']
            d = p['dist']
            if d >= 1000:
            ...
            rows_srv.append(
                f'<tr>'
                f'<td>{p["name"]}</td>'
                f'<td style="color:#5a7a96;">{cat_lbl}</td>'
                f'<td style="text-align:right;font-weight:600;color:#243448;">{dist_str}</td>'
                f'</tr>'
            )
        srv_table = (
            f'<div class="card stone" style="padding:3mm 5mm; margin-top:0; overflow:hidden;">'
            f'<h4 style="margin-bottom:2mm;">{srv_lbl}</h4>'
            f'<table class="dtable" style="font-size:7.5pt;">'
            f'<tbody>{header_row}{"".join(rows_srv[:10])}</tbody>'
            f'</table>'
            f'</div>'
        )
    else:
    ...
    return _page(
        f'<div class="inner">'
        f'{_sh(sh_ey, sh_tt, n)}'
        f'<div class="card stone" style="padding:3mm 5mm; margin-bottom:3mm;">'
        f'<p class="text" style="margin:0; line-height:1.55;">{lead}</p>'
        f'{body}'
        f'</div>'
        f'<div class="g2" style="align-items:start; margin-bottom:3mm;">'
        f'  <div class="card sky" style="padding:3mm 4mm;">'
        f'    <h4>{radar_lbl}</h4>'
        f'    <div style="margin-top:2mm; display:flex; justify-content:center;">{radar_svg}</div>'
        f'  </div>'
        f'  <div class="card" style="padding:3mm 4mm;">'
        f'    <h4>{bars_lbl}</h4>'
        f'    <div class="bars-block" style="margin-top:2mm;">{bars_html}</div>'
        f'  </div>'
        f'</div>'
        f'{map_html}'
        f'{srv_table}'
...
def _build_services(data, content, lang, n):
    ...
    if es:
        ...
        _def_intro = (f"El entorno de {barrio} ofrece un ecosistema completo de servicios que potencia "
                      f"el valor del activo y la calidad de vida de sus ocupantes.")
        intro = _ai(content, 'services_intro', _def_intro)

        svc_blocks = [
            ('Transporte y Movilidad',
             _ai(content, 'services_transport', "Red de transporte con acceso a metro, bus y cercanias."),
             f"{ls.get('transporte', 7.0)}/10"),
            ('Educacion',
             _ai(content, 'services_education', "Centros educativos de primer nivel en el entorno inmediato."),
             f"{ls.get('servicios', 7.0)}/10"),
            ('Servicios Sanitarios',
             _ai(content, 'services_health', "Cobertura sanitaria completa con hospitales y clinicas privadas."),
             f"{min(10, float(ls.get('servicios', 7.0)) + 0.5):.1f}/10"),
            ('Comercio y Restauracion',
             _ai(content, 'services_commerce', "Tejido comercial activo con restaurantes y zonas de ocio."),
             f"{ls.get('comercios', 7.0)}/10"),
            ('Zonas Verdes',
             _ai(content, 'services_green', "Parques y jardines accesibles a pie desde el inmueble."),
             f"{ls.get('conectividad', 7.5):.1f}/10"),
            ('Seguridad y Entorno',
             f"Barrio consolidado de {barrio} con perfil residencial estable y entorno urbano cuidado.",
             f"{ls.get('atractivo_residencial', 7.5):.1f}/10"),
        ]
        conclusion = _ai(content, 'services_conclusion',
                         f"La dotacion global de servicios en {barrio} alcanza una puntuacion de "
                         f"{ls.get('servicios', 7.5)}/10, respaldando la solidez de la inversion.")
    else:
        sh_ey = 'Services & Infrastructure'; sh_tt = f'Everything that defines quality of life in {barrio}.'
        intro = _ai(content, 'services_intro',
                    f"The surroundings of {barrio} offer a complete urban services ecosystem.")

        svc_blocks = [
            ('Transport & Mobility',
             _ai(content, 'services_transport', "Public transport with metro, bus and commuter rail access."),
             f"{ls.get('transporte', 7.0)}/10"),
            ('Education',
             _ai(content, 'services_education', "First-class educational centres in the immediate vicinity."),
             f"{ls.get('servicios', 7.0)}/10"),
            ('Healthcare',
             _ai(content, 'services_health', "Full healthcare coverage with hospitals and private clinics."),
             f"{min(10, float(ls.get('servicios', 7.0)) + 0.5):.1f}/10"),
            ('Commerce & Dining',
             _ai(content, 'services_commerce', "Active commercial fabric with restaurants and leisure areas."),
             f"{ls.get('comercios', 7.0)}/10"),
            ('Green Spaces',
             _ai(content, 'services_green', "Parks and gardens walkable from the property."),
             f"{ls.get('conectividad', 7.5):.1f}/10"),
            ('Safety & Urban Quality',
             f"Consolidated neighbourhood of {barrio} with stable residential profile.",
             f"{ls.get('atractivo_residencial', 7.5):.1f}/10"),
        ]
        conclusion = _ai(content, 'services_conclusion',
                         f"The global services score in {barrio} reaches {ls.get('servicios', 7.5)}/10.")

    # ── Gráfico de barras de puntuaciones de servicios ───────────────────────
    bar_labels = (['Transporte', 'Servicios', 'Comercios', 'Conectividad', 'Calidad Residencial'] if es else
                  ['Transport', 'Services', 'Commerce', 'Connectivity', 'Residential Quality'])
    bar_vals = [
        min(10.0, max(0.0, float(ls.get('transporte', 7.0)))),
        min(10.0, max(0.0, float(ls.get('servicios', 7.0)))),
        min(10.0, max(0.0, float(ls.get('comercios', 7.0)))),
        min(10.0, max(0.0, float(ls.get('conectividad', 7.5)))),
        min(10.0, max(0.0, float(ls.get('atractivo_residencial', 7.5)))),
    ]
    _bl, _bw, _brh, _bgap = 74, 306, 12, 5
    _n = len(bar_vals)
    _sh2 = 6 + _n * (_brh + _bgap) - _bgap + 6
    _sw = _bl + _bw + 34
    _bar_svg = ''
    for _i, (_lbl, _v) in enumerate(zip(bar_labels, bar_vals)):
        _by = 6 + _i * (_brh + _bgap)
        _bfill = _v / 10.0 * _bw
        _col = '#c9a84c' if _v >= 8.0 else ('#4d8aab' if _v >= 6.5 else '#8aacbf')
        _bar_svg += (
            f'<text x="{_bl - 4}" y="{_by + _brh * 0.72:.1f}" text-anchor="end" '
            f'font-size="7" fill="#4d6275" font-family="Inter,Arial,sans-serif">{_lbl}</text>'
            f'<rect x="{_bl}" y="{_by}" width="{_bw}" height="{_brh}" rx="2" fill="rgba(201,168,76,0.10)"/>'
            f'<rect x="{_bl}" y="{_by}" width="{_bfill:.1f}" height="{_brh}" rx="2" fill="{_col}" opacity="0.75"/>'
            f'<text x="{_bl + _bfill + 5:.1f}" y="{_by + _brh * 0.72:.1f}" font-size="7.5" font-weight="800" '
            f'fill="#1e2f40" font-family="Inter,Arial,sans-serif">{_v:.1f}</text>'
        )
    _chart_lbl = 'Análisis de Servicios y Entorno Urbano' if es else 'Urban Services & Environment Analysis'
    score_chart_html = (
        f'<div style="margin:3mm 0; background:rgba(251,248,242,0.9); '
        f'border:1px solid rgba(201,168,76,0.25); padding:3mm 4mm 3mm;">'
        f'<div class="micro" style="margin-bottom:2mm; color:#1e2f40; font-weight:600;">{_chart_lbl}</div>'
        f'<svg viewBox="0 0 {_sw} {_sh2}" xmlns="http://www.w3.org/2000/svg" '
        f'width="100%" style="display:block;">{_bar_svg}</svg>'
        f'</div>'
    )

    # Nearby places del OSM para enriquecer la página de servicios
    nearby = content.get('nearby_places', [])
    nearby_rows = ''
    if nearby:
        dist_lbl = 'Dist.' if es else 'Dist.'
        tipo_lbl = 'Tipo' if es else 'Type'
        name_lbl = 'Lugar / Servicio' if es else 'Place / Service'
        nearby_rows = (
            f'<div class="card sky" style="padding:3mm 5mm;margin-bottom:3mm;">'
            f'<h4 style="margin-bottom:2mm;">{"Puntos de interés confirmados (OpenStreetMap)" if es else "Confirmed points of interest (OpenStreetMap)"}</h4>'
            f'<table class="dtable" style="font-size:7.5pt;">'
            f'<thead><tr><th style="width:45%">{name_lbl}</th><th style="width:30%">{tipo_lbl}</th><th style="width:25%;text-align:right">{dist_lbl}</th></tr></thead>'
            f'<tbody>' +
            ''.join(
                f'<tr><td>{p["name"]}</td>'
                f'<td style="color:#5a7a96">{p["cat_es"] if es else p["cat_en"]}</td>'
                f'<td style="text-align:right;font-weight:600">'
                f'{"%.0f m" % p["dist"] if p["dist"] < 1000 else "%.1f km" % (p["dist"]/1000)}'
                f'</td></tr>'
                for p in nearby[:8]
            ) +
            f'</tbody></table></div>'
    ...
    # Cards — texto limitado a 2 líneas para no desbordar
    svc_cards_html = ''.join(
        f'<div class="svc-card">'
        f'<h4 style="font-size:7.5pt;">{lbl}</h4>'
        f'<p class="text" style="font-size:7pt;line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;">{desc}</p>'
    ...
    return _page(
        f'<div class="inner">'
        f'{_sh(sh_ey, sh_tt, n)}'
        f'<p class="lead" style="margin-bottom:3mm;">{intro}</p>'
        f'<div class="g3" style="margin-bottom:3mm;gap:3mm;">{svc_cards_html}</div>'
        f'{score_chart_html}'
        f'{nearby_rows}'
        f'<div class="note" style="margin-top:2mm;">{conclusion}</div>'
        f'{_foot(sh_ey, ciudad)}'
        f'</div>',
        bg_key='services'
    )
...
def _build_market(data, content, lang, n):
    es = lang == 'es'
    ciudad  = _s(data.get('ciudad'), '')
    barrio  = _s(data.get('barrio')) or ciudad
    precio  = float(data.get('precio_venta') or 0)
    m2_val  = float(data.get('metros_construidos') or 0)
    pm2_zona = float(data.get('precio_m2_zona') or 0)
    pm2_prop = int(precio / m2_val) if m2_val > 0 and precio > 0 else 0

    if es:
        sh_ey = 'Analisis de Mercado'; sh_tt = f'Competencia, posicionamiento y oportunidad en {barrio}.'
        comp_header = '<tr><th>Tipo</th><th>Ubicaci&oacute;n</th><th>Precio</th><th>Sup.</th><th>Comparativa</th></tr>'
        lbl_pos  = 'Posicionamiento Competitivo'
        lbl_opp  = 'Ventana de Oportunidad'
        lbl_comp = 'Propiedades Comparables en el Mercado'
        lbl_pm2  = 'Precio/m² de este activo vs media de zona'
    else:
        sh_ey = 'Market Analysis'; sh_tt = f'Competition, positioning and opportunity in {barrio}.'
        comp_header = '<tr><th>Type</th><th>Location</th><th>Price</th><th>Area</th><th>Comparison</th></tr>'
        lbl_pos  = 'Competitive Positioning'
        lbl_opp  = 'Window of Opportunity'
        lbl_comp = 'Comparable Properties on the Market'
        lbl_pm2  = 'Price/m² of this asset vs area average'

    # Fallbacks si Claude no está disponible
    fb_context = (f"{barrio} es una zona con alta demanda y oferta limitada de inmuebles de calidad. "
                  f"El mercado muestra tendencia alcista consolidada con potencial de revalorización." if es else
                  f"{barrio} shows high demand and limited quality supply. The market displays a consolidated upward trend with appreciation potential.")
    fb_pos = (f"El activo se posiciona en el segmento premium de {barrio}, con precio competitivo respecto a comparables de la zona." if es else
              f"The asset positions itself in the premium segment of {barrio}, competitively priced against local comparables.")
    fb_opp = (f"Momento óptimo de entrada: mercado con liquidez creciente y demanda sostenida. Oportunidad de capturar plusvalía en el corto-medio plazo." if es else
              f"Optimal entry moment: market with growing liquidity and sustained demand. Opportunity to capture capital gains in the short-to-medium term.")

    mkt_context    = _ai(content, 'market_context', fb_context)
    mkt_positioning = _ai(content, 'market_positioning', fb_pos)
    mkt_opportunity = _ai(content, 'market_opportunity', fb_opp)
    comparables    = (content.get('ai_content') or {}).get('market_comparable', [])

    # ── Tabla de comparables ─────────────────────────────────────────────────
    # Fallback si Claude no generó comparables
    if not comparables and pm2_prop > 0:
        delta = int(pm2_prop * 0.05)
        tipo_str = data.get('tipo_propiedad', 'Apartamento').capitalize()
        comparables = [
            {'tipo': tipo_str, 'ubicacion': barrio, 'precio': f'{int(pm2_prop * float(data.get("metros_construidos", 70)):,} €'.replace(",","."), 'm2': data.get('metros_construidos', '—'), 'nota': 'Precio similar zona'},
            {'tipo': tipo_str, 'ubicacion': f'{barrio} — reformado', 'precio': f'{int((pm2_prop + delta) * float(data.get("metros_construidos", 70))):,} €'.replace(",","."), 'm2': data.get('metros_construidos', '—'), 'nota': f'+5% vs este activo'},
            {'tipo': tipo_str, 'ubicacion': f'{ciudad} — zona adyacente', 'precio': f'{int((pm2_prop - delta) * float(data.get("metros_construidos", 70))):,} €'.replace(",","."), 'm2': data.get('metros_construidos', '—'), 'nota': 'Menor calidad de zona'},
        ]
    ...
    if comparables:
        for c in comparables[:4]:
            comp_rows += (
                f'<tr>'
                f'<td>{c.get("tipo","")}</td>'
                f'<td>{c.get("ubicacion","")}</td>'
                f'<td style="font-weight:600;color:#c9a84c;">{c.get("precio","")}</td>'
                f'<td>{c.get("m2","")}</td>'
                f'<td style="font-style:italic;color:#5a7a96;font-size:7pt;">{c.get("nota","")}</td>'
                f'</tr>'
            )
    comp_section = (
        f'<div class="card stone" style="padding:3mm 5mm;margin-bottom:3mm;">'
        f'<h4 style="margin-bottom:2mm;">{lbl_comp}</h4>'
        f'<table class="dtable" style="font-size:7.5pt;">'
        f'<tbody>{comp_header}{comp_rows}</tbody>'
        f'</table>'
        f'</div>'
    ) if comp_rows else ''

    # ── Gráfico precio/m² vs zona ────────────────────────────────────────────
    price_chart = ''
    if pm2_prop > 0 and pm2_zona > 0:
        max_val = max(pm2_prop, pm2_zona) * 1.15
        w_prop  = int(pm2_prop / max_val * 100)
        w_zona  = int(pm2_zona / max_val * 100)
        pm2_prop_fmt = f"{pm2_prop:,}".replace(",", ".") + " €/m²"
        pm2_zona_fmt = f"{int(pm2_zona):,}".replace(",", ".") + " €/m²"
        lbl_this = 'Este activo' if es else 'This asset'
        lbl_avg  = 'Media zona' if es else 'Area avg'
        price_chart = (
            f'<div style="margin-bottom:3mm;">'
            f'<div class="micro" style="margin-bottom:2mm;font-weight:600;">{lbl_pm2}</div>'
            f'<div style="margin-bottom:1.5mm;display:flex;align-items:center;gap:3mm;">'
            f'  <span class="micro" style="width:20mm;text-align:right;">{lbl_this}</span>'
            f'  <div style="flex:1;background:rgba(36,52,71,0.08);border-radius:1mm;height:5mm;">'
            f'    <div style="width:{w_prop}%;height:100%;background:linear-gradient(90deg,#c9a84c,#e8c96a);border-radius:1mm;"></div>'
            f'  </div>'
            f'  <span class="micro" style="width:22mm;font-weight:700;">{pm2_prop_fmt}</span>'
            f'</div>'
            f'<div style="display:flex;align-items:center;gap:3mm;">'
            f'  <span class="micro" style="width:20mm;text-align:right;">{lbl_avg}</span>'
            f'  <div style="flex:1;background:rgba(36,52,71,0.08);border-radius:1mm;height:5mm;">'
            f'    <div style="width:{w_zona}%;height:100%;background:rgba(90,122,150,0.5);border-radius:1mm;"></div>'
            f'  </div>'
            f'  <span class="micro" style="width:22mm;">{pm2_zona_fmt}</span>'
            f'</div>'
            f'</div>'
        )
    ...
    return _page(
        ...
        f'{_sh(sh_ey, sh_tt, n)}'
        + (f'<div class="card stone" style="padding:3mm 5mm;margin-bottom:3mm;">'
           f'<p class="text" style="margin:0;line-height:1.6;">{mkt_context}</p>'
           f'</div>' if mkt_context else '') +
        f'{price_chart}'
        f'{comp_section}'
        f'<div class="g2" style="margin-top:0;">'
        f'{_card(lbl_pos, "", mkt_positioning, "sky")}'
        f'{_card(lbl_opp, "", mkt_opportunity, "dark")}'
        f'</div>'
        f'{_foot(sh_ey, ciudad)}'
        f'</div>',
        bg_key='investment'
...
def _build_investment(data, content, lang, n):
    ...
    lead = _ai(content, 'financial_narrative', paras[0] if paras else '')
    ...
    donut1 = _svg_donut(min(yb_float * 10, 100), f"{yb_float:.1f}%", 78)
    donut2 = _svg_donut(min(yn_float * 10, 100), f"{yn_float:.1f}%", 78, '#8a6828')
    donut3 = _svg_donut(min(roi_float * 3, 100),  f"{roi_float:.1f}%", 78, '#1e3248')

    # Payback: solo mostrar si es razonable (< 30 años y hay datos de renta)
    pb_val = float(pb) if pb else 0
    pb_str = (f"{pb_val:.1f} {'años' if es else 'years'}" if 0 < pb_val <= 30 else
              ('Sin datos de renta' if es else 'No rental data'))
    ...
    if es:
        ...
        rows = [
            ...
            ('Ingresos brutos anuales', _eur(ing) if ing else '—'),
            ...
            ('Periodo de recuperacion', pb_str if ing else '—'),
        ]
        ...
        nota = (f"Escenario calculado con {_s(data.get('ocupacion','90'))}% de ocupacion y revalorizacion "
                f"estimada del {_pct(rev)}/ano. Datos orientativos — revisar con asesor financiero.")
    else:
        ...
        rows = [
            ...
            ('Annual gross income', _eur(ing) if ing else '—'),
            ...
            ('Payback period', pb_str if ing else '—'),
        ]
    ...
    donuts_html = (
        f'<div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:2mm; margin:3mm 0;">'
        f'  <div style="text-align:center; padding:3mm 2mm; background:linear-gradient(135deg,var(--warm),var(--pearl)); border:1px solid rgba(201,168,76,0.28);">'
    ...
    if precio > 0:
        rev_rate = rev / 100.0
        proj_vals = [precio * ((1 + rev_rate) ** y) for y in range(6)]  # año 0..5
        max_v = max(proj_vals)
        min_v = min(proj_vals)
        rng   = max_v - min_v if max_v != min_v else 1
        W, H, pad_l, pad_r, pad_t, pad_b = 420, 62, 42, 8, 6, 18
        chart_w = W - pad_l - pad_r
        chart_h = H - pad_t - pad_b
        def px(i): return pad_l + i * chart_w / 5
        def py(v): return pad_t + chart_h - (v - min_v) / rng * chart_h
        pts = ' '.join(f'{px(i):.1f},{py(v):.1f}' for i, v in enumerate(proj_vals))
        area_pts = (f'{px(0):.1f},{pad_t + chart_h} ' + pts +
                    f' {px(5):.1f},{pad_t + chart_h}')
        dots = ''.join(
            f'<circle cx="{px(i):.1f}" cy="{py(v):.1f}" r="3.5" fill="#c9a84c"/>'
            for i, v in enumerate(proj_vals)
        )
        xlabels = ''.join(
            f'<text x="{px(i):.1f}" y="{H - 4}" text-anchor="middle" '
            f'font-size="7" fill="#4d6275">{"Año" if es else "Y"}{i}</text>'
            for i in range(6)
        )
        ylabels = ''.join(
            f'<text x="{pad_l - 4}" y="{py(min_v + rng * t / 3) + 2:.1f}" text-anchor="end" '
            f'font-size="6.5" fill="#4d6275">{_eur(int(min_v + rng * t / 3))}</text>'
            for t in range(4)
        )
        chart_title = f'{"Proyeccion de Valor a 5 Anos" if es else "5-Year Value Projection"}'
        proj_svg = (
            f'<div style="margin:2mm 0 1mm; background:rgba(251,248,242,0.9); '
            f'border:1px solid rgba(201,168,76,0.28); padding:2mm 4mm;">'
            f'<div class="micro" style="margin-bottom:2mm; color:#1e2f40; font-weight:600;">{chart_title}</div>'
            f'<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" '
            f'width="100%" style="display:block;">'
            f'<defs><linearGradient id="projGrad" x1="0" y1="0" x2="0" y2="1">'
            f'<stop offset="0%" stop-color="#c9a84c" stop-opacity="0.25"/>'
            f'<stop offset="100%" stop-color="#c9a84c" stop-opacity="0.02"/>'
            f'</linearGradient></defs>'
            f'<polygon points="{area_pts}" fill="url(#projGrad)"/>'
            f'<polyline points="{pts}" fill="none" stroke="#c9a84c" stroke-width="2" stroke-linejoin="round"/>'
            f'{dots}{xlabels}{ylabels}'
            f'</svg>'
            f'</div>'
        )
    ...
    return _page(
        ...
        f'{_sh(sh_ey, sh_tt, n)}'
        f'<p class="lead" style="margin-bottom:3mm;">{lead}</p>'
        f'<div class="divider" style="margin:3mm 0;"></div>'
...
def _build_advantages(data, content, lang, n):
    ...
    # Usar ventajas de Claude si están disponibles
    ai_advantages = (content.get('ai_content') or {}).get('advantages', [])
    if ai_advantages:
        args = ai_advantages[:6]
    else:
        while len(args) < 6:
    ...
    concl = _ai(content, 'conclusion', content['conclusions'].get('texto', ''))
...
def _build_value_plan(data, content, lang, n):
    ...
    # Contexto de IA para adaptar el mensaje de apertura
    ai_tesis = _ai(content, 'investment_thesis', '')
    ai_demand = _ai(content, 'investment_demand', '')

    if es:
        sh_ey = 'Plan de Creacion de Valor'; sh_tt = 'Como convertir este activo en rentabilidad sostenida.'
        fases = [
            ('Fase 1','Adquisicion',
             ai_demand[:180] if ai_demand else 'Due diligence completa, negociacion y cierre en condiciones optimas de mercado.'),
            ('Fase 2','Optimizacion','Reforma estrategica o puesta a punto del activo segun el estado actual. Posicionamiento diferencial en el segmento de mercado correspondiente.'),
            ('Fase 3','Monetizacion','Comercializacion con pricing competitivo basado en el analisis de comparables de zona. Seleccion rigurosa de inquilino o comprador optimo.'),
            ('Fase 4','Consolidacion','Gestion activa del activo. Revision anual de rentabilidad y analisis continuo de la estrategia de salida mas adecuada.'),
        ]
        extras = [
        ...
        fases = [
            ('Phase 1','Acquisition',
             ai_demand[:180] if ai_demand else 'Full due diligence, negotiation and closing under optimal market conditions.'),
            ('Phase 2','Optimisation','Strategic renovation or asset preparation based on current condition. Differential positioning within the relevant market segment.'),
            ('Phase 3','Monetisation','Competitive pricing based on comparable analysis in the area. Rigorous selection of optimal tenant or buyer.'),
            ('Phase 4','Consolidation','Active asset management. Annual return review and continuous analysis of the most appropriate exit strategy.'),
        ]
        extras = [
    ...
    # Nota de apertura con tesis de Claude
    tesis_note = f'<div class="note" style="margin-bottom:4mm;">{ai_tesis}</div>' if ai_tesis else ''
...
def _build_strategic(data, content, lang, n):
    ...
    # Tesis de inversión desde Claude
    tesis = _ai(content, 'investment_thesis', '')
    tesis_html = f'<div class="note" style="margin-bottom:4mm;">{tesis}</div>' if tesis else ''

    # Demanda, apreciación y riesgos desde Claude o fallback template
    ai_demand      = _ai(content, 'investment_demand', '')
    ai_appreci     = _ai(content, 'investment_appreciation', '')
    ai_risks_text  = _ai(content, 'investment_risks', '')

    if es:
        demand_lbl = 'Demanda del Mercado'; appreci_lbl = 'Potencial de Apreciacion'; risks_lbl = 'Riesgos a Considerar'
        opp_lbl = 'Oportunidades'; rec_lbl = 'Recomendaciones'
    else:
        demand_lbl = 'Market Demand'; appreci_lbl = 'Appreciation Potential'; risks_lbl = 'Risks to Consider'
        opp_lbl = 'Opportunities'; rec_lbl = 'Recommendations'

    # Cards de análisis AI (demanda + apreciación + riesgos)
    if ai_demand or ai_appreci or ai_risks_text:
        analysis_html = (
            f'<div class="g3">'
            f'{_card(demand_lbl, "", ai_demand, "sky") if ai_demand else ""}'
            f'{_card(appreci_lbl, "", ai_appreci, "stone") if ai_appreci else ""}'
            f'{_card(risks_lbl, "", ai_risks_text, "dark") if ai_risks_text else ""}'
            f'</div>'
        )
    else:
        # Fallback: cards de riesgos del template
        variants = ['stone', 'sky', 'gold']
        analysis_html = (
            f'<div class="g3">'
            + ''.join(_card(item[0], item[1].upper(), item[2], variants[i % len(variants)])
...
def _build_lifestyle(data, content, lang, n):
    ...
    # Usar texto de Claude si disponible
    lead = _ai(content, 'lifestyle_intro', paras[0] if paras else '')
    ai_body = _ai(content, 'lifestyle_body', '')
    if ai_body:
        body = f'<p class="text" style="margin-bottom:2.5mm;">{ai_body}</p>'
    else:
    ...
    # Nearby places del análisis OSM para lifestyle
    nearby = content.get('nearby_places', [])
    if nearby:
        nearby_rows = ''.join(
            f'<tr><td>{p.get("nombre","")}</td><td>{p.get("tipo","")}</td><td>{p.get("distancia_m","")}</td></tr>'
            for p in nearby[:8]
        )
        dist_lbl = 'Distancia (m)' if es else 'Distance (m)'
        type_lbl = 'Tipo' if es else 'Type'
        name_lbl = 'Lugar' if es else 'Place'
        nearby_html = (
            f'<h4 style="margin-bottom:2mm;">{"Lugares Destacados" if es else "Highlighted Places"}</h4>'
            f'<table class="dtable"><thead><tr><th>{name_lbl}</th><th>{type_lbl}</th><th>{dist_lbl}</th></tr></thead>'
            f'<tbody>{nearby_rows}</tbody></table>'
        )
    else:
        zone_imgs = _zone_imgs(data, lang)
        nearby_html = '<div class="vgallery">' + ''.join(
            f'<div class="vitem"><img src="{u}" alt="{l}"/><div class="vcap">{l}</div></div>'
            for u, l in zone_imgs[:4]
        ) + '</div>'
...
def _build_final(data, content, lang):
    ...
    if es:
        if tipo_dossier == 'inversores':
            title_fallback = f"Una oportunidad limitada para tomar posicion en {ciudad}."
            copy_fallback  = f"Este activo representa una ventana de inversion con fundamentos solidos en {barrio}. Las oportunidades de este calibre son escasas y de alta rotacion en el mercado actual."
            cta_fallback   = "Solicite su visita privada y documentacion tecnica completa."
        else:
            title_fallback = f"Tu hogar en {barrio} te esta esperando."
            copy_fallback  = f"Cada espacio de este inmueble ha sido seleccionado para ofrecerte calidad de vida superior en {ciudad}. Da el primer paso hoy."
            cta_fallback   = "Solicite una visita privada para conocer el inmueble en persona."
        ...
        if tipo_dossier == 'inversores':
            title_fallback = f"A limited opportunity to take position in {ciudad}."
            copy_fallback  = f"This asset represents an investment window with solid fundamentals in {barrio}. Opportunities of this calibre are scarce and high-turnover in today's market."
            cta_fallback   = "Request your private visit and complete technical documentation."
        else:
            title_fallback = f"Your home in {barrio} is waiting for you."
            copy_fallback  = f"Every space in this property has been selected to offer you superior quality of life in {ciudad}. Take the first step today."
            cta_fallback   = "Request a private visit to see the property in person."
    ...
    # Usar texto de Claude si está disponible
    ai_conclusion = _ai(content, 'conclusion', '')
    ai_cta        = _ai(content, 'cta', '')
    title = ai_conclusion if ai_conclusion else title_fallback
    copy  = copy_fallback
    cta   = ai_cta if ai_cta else cta_fallback
...
def _build_html(data, content, lang):
    ...
    comm_pages = _build_commercial(data, content, lang, n)
    if isinstance(comm_pages, list):
        pages.extend(comm_pages); n += len(comm_pages)
    else:
        pages.append(comm_pages); n += 1
    pages.append(_build_location(data, content, lang, n));           n += 1
    pages.append(_build_services(data, content, lang, n));           n += 1
    pages.append(_build_market(data, content, lang, n));             n += 1
