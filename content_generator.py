...
# ─── LUGARES CERCANOS VÍA OPENSTREETMAP ──────────────────────────────────────
def _fetch_nearby_places(data):
    """
    Geocodifica la dirección con Nominatim y consulta Overpass API para obtener
    lugares reales cercanos (metro, colegios, hospitales, supermercados, parques).
    Devuelve lista de dicts: {name, cat_es, cat_en, dist}
    """
    try:
        import requests
    except ImportError:
    ...
    parts = [p for p in [direccion, barrio, ciudad, cp, 'España'] if p]
    if not parts:
        return []
    ...
    try:
        geo = requests.get(
            ...
            params={'q': query_str, 'format': 'json', 'limit': 1, 'countrycodes': 'es'},
            headers={'User-Agent': 'InfrAltum-Dossier/1.0 contactoinfraltum@gmail.com'},
            timeout=6
        ).json()
        if not geo:
            return []
...
""".strip()
    ...
    # Prioridad de categorías (primera coincidencia gana)
    CATEGORIES = [
        (('railway', 'station'),            'Estación de tren',      'Train station'),
        (('railway', 'subway_entrance'),     'Metro',                 'Metro'),
        (('railway', 'halt'),               'Apeadero',              'Halt'),
        (('amenity', 'bus_station'),         'Estación de autobús',   'Bus station'),
        (('highway', 'bus_stop'),            'Parada de autobús',     'Bus stop'),
        (('amenity', 'hospital'),            'Hospital',              'Hospital'),
        (('amenity', 'clinic'),              'Clínica',               'Clinic'),
        (('amenity', 'pharmacy'),            'Farmacia',              'Pharmacy'),
        (('amenity', 'university'),          'Universidad',           'University'),
        (('amenity', 'college'),             'Centro formativo',      'College'),
        (('amenity', 'school'),              'Colegio',               'School'),
        (('shop', 'mall'),                   'Centro comercial',      'Shopping mall'),
        (('shop', 'supermarket'),            'Supermercado',          'Supermarket'),
        (('leisure', 'park'),                'Parque',                'Park'),
        (('leisure', 'garden'),              'Jardín',                'Garden'),
        (('amenity', 'restaurant'),          'Restaurante',           'Restaurant'),
    ]
    ...
    places = []
    seen_names = set()
    cat_count  = {}   # cat_es -> count
    CAT_LIMIT  = {'Restaurante': 2, 'Restaurant': 2, 'Farmacia': 2, 'Pharmacy': 2,
                  'Parada de autobús': 2, 'Bus stop': 2, 'Colegio': 3, 'School': 3}
    ...
    for el in res.get('elements', []):
        ...
        cat_es = cat_en = None
        for (k, v), l_es, l_en in CATEGORIES:
            if tags.get(k) == v:
                cat_es, cat_en = l_es, l_en
                break
        if not cat_es:
            continue

        limit = CAT_LIMIT.get(cat_es, 2)
        if cat_count.get(cat_es, 0) >= limit:
            continue
        ...
        seen_names.add(name)
        cat_count[cat_es] = cat_count.get(cat_es, 0) + 1
        places.append({
            'name': name, 'cat_es': cat_es, 'cat_en': cat_en,
            'dist': dist, 'lat': el_lat, 'lon': el_lon
        })

    # Ordenar por distancia, máximo 10 resultados
    places.sort(key=lambda x: x['dist'])
    return places[:10], lat, lon
...
# ─── MAPA ESTÁTICO OSM ───────────────────────────────────────────────────────
def _generate_map_b64(prop_lat, prop_lon, nearby_places, width=560, height=300):
    """Genera un mapa OSM estático con el inmueble marcado en rojo y los lugares en azul."""
    try:
        ...
        m = StaticMap(width, height,
                      ...
                      headers={'User-Agent': 'InfrAltum-Dossier/1.0'})

        # Lugares cercanos — círculo azul oscuro
        for p in nearby_places:
            p_lat = p.get('lat')
            p_lon = p.get('lon')
            if p_lat and p_lon:
                m.add_marker(CircleMarker((p_lon, p_lat), '#4a6fa5', 9))

        # Inmueble — círculo rojo prominente
        m.add_marker(CircleMarker((prop_lon, prop_lat), '#c0392b', 16))
...
def gen_commercial_narrative(data, fin, premium_score, lang='es'):
    ...
    def _shorten(text, max_chars=1200):
        """Keep the most informative sentences within max_chars."""
        import re
        # Split into sentences
        sentences = [s.strip() for s in re.split(r'(?<=[.!?])\s+', text) if s.strip()]
        if not sentences:
            return text
        # Score each sentence by informativeness (length is a proxy; prefer specific ones)
        def score(s):
            s_lower = s.lower()
            bonus = sum(1 for kw in ['m²','dormitorio','baño','terraza','jardín','piscina','cocina','salon','garaje','precio','luminoso','reformado','nuevo','exterior','interior','vistas','equipado','calidad','lujo','diseño','moderno','amplio'] if kw in s_lower)
            return bonus * 10 + min(len(s), 80)
        ranked = sorted(enumerate(sentences), key=lambda x: score(x[1]), reverse=True)
        # Pick sentences greedily by score until we hit the char limit
        chosen = []
        total = 0
        ...
        if not result:
            ...
            result = sentences[0][:max_chars].rstrip(',') + '.'
        return result
    ...
    if lang == 'es':
        ...
        if descripcion:
            ...
            limit = 1150  # chars available below images (p3 slot)
            if len(desc) > limit:
                cut = desc[:limit]
                last_dot = cut.rfind('.')
                desc = cut[:last_dot + 1] if last_dot > limit // 2 else cut.rstrip() + '.'
            if not desc.endswith('.'): desc += '.'
            p3 = f"En cuanto a sus características: {desc[0].lower()}{desc[1:]}"
        else:
        ...
        if feat_list:
            feats_str = ', '.join(feat_list[:-1]) + (' y ' + feat_list[-1] if len(feat_list) > 1 else feat_list[0])
            p4 = f"Destacan: {feats_str}."
        else:
            p4 = "Configuración completa pensada para maximizar el confort de sus ocupantes."
        return p1 + "\n\n" + p2 + "\n\n" + p3 + "\n\n" + p4
    else:
        ...
        if descripcion:
            ...
            if len(desc) > limit:
                ...
                desc = cut[:last_dot + 1] if last_dot > limit // 2 else cut.rstrip() + '.'
            if not desc.endswith('.'): desc += '.'
            p3 = f"As for its features: {desc[0].lower()}{desc[1:]}"
        else:
            p3 = "The construction qualities and finishes meet the highest standard in the segment, with special attention to detail and occupant comfort."
        if feat_list:
            feats_str = ', '.join(feat_list[:-1]) + (' and ' + feat_list[-1] if len(feat_list) > 1 else feat_list[0])
            p4 = f"Highlights: {feats_str}."
        else:
            p4 = "Complete configuration designed to maximise comfort and the experience of its occupants."
        return p1 + "\n\n" + p2 + "\n\n" + p3 + "\n\n" + p4
...
# ─── GENERACIÓN IA CON CLAUDE API ────────────────────────────────────────────
def _gen_with_claude(data, nearby_places, fin, loc_scores, lang):
    """
    Llama a Claude API para generar todo el contenido analítico del dossier.
    Devuelve un dict con todos los campos de texto, o None si la API no está disponible.
    """
    import os
    ...
    tipo_prop    = data.get('tipo_propiedad', '')
    estado       = data.get('estado', '')
    anyo         = data.get('anyo_construccion', '')
    direccion    = data.get('direccion', '')
    barrio       = data.get('barrio', '') or data.get('ciudad', '')
    ciudad       = data.get('ciudad', '')
    ...
    m2           = data.get('metros_construidos', '')
    m2_utiles    = data.get('metros_utiles', '')
    m2_parcela   = data.get('metros_parcela', '')
    dorms        = data.get('dormitorios', '')
    banos        = data.get('banos', '')
    descripcion  = (data.get('descripcion') or '').strip()
    pm2_zona     = data.get('precio_m2_zona', '')
    demand       = data.get('demanda_alquiler', '')
    revalor      = data.get('revalorizacion', '')
    tendencia    = data.get('tendencia_mercado', '')
    perfil       = data.get('perfil_compradores', '')
    info_inv     = data.get('info_adicional_inversores', '')
    renta        = data.get('renta_mensual', '')
    ocupacion    = data.get('ocupacion', '90')

    feats = data.get('caracteristicas', [])
    if isinstance(feats, str):
        try:
            import json as _j; feats = _j.loads(feats)
        except: feats = []
    feats_str = ', '.join(feats) if feats else 'no especificadas'

    nearby_str = '\n'.join(
        f"  - {p['name']} ({p.get('cat_es', '')}): {p['dist']}m"
        for p in nearby_places[:10]
    ) if nearby_places else 'No disponible'
    ...
    idioma_txt = 'español profesional e impecable' if es else 'professional, impeccable English'

    prompt = f"""Eres el mejor analista inmobiliario de España, especializado en activos prime y de alto standing. Llevas 25 años asesorando a family offices, inversores institucionales y compradores de ultra-lujo. Tus informes son los más precisos y convincentes del mercado.

INMUEBLE:
- Tipo: {tipo_prop} | Estado: {estado} | Año construcción: {anyo}
- Dirección: {direccion}, {barrio}, {ciudad}{', ' + provincia if provincia else ''}
- Precio: {precio} € | Superficie: {m2} m²{' construidos, ' + m2_utiles + ' m² útiles' if m2_utiles else ''}{', parcela ' + m2_parcela + ' m²' if m2_parcela else ''}
- Habitaciones: {dorms} | Baños: {banos}
- Características: {feats_str}
- Descripción del propietario (MEJORA Y AMPLÍA ESTA DESCRIPCIÓN, no hagas copia-pega): "{descripcion}"
- Precio m² de la zona: {pm2_zona if pm2_zona else 'no indicado'} €/m²

DATOS DE MERCADO:
- Demanda alquiler: {demand} | Revalorización: {revalor} | Tendencia: {tendencia or 'no indicada'}
- Perfil comprador habitual: {perfil or 'no indicado'}
- Info adicional: {info_inv or 'ninguna'}

DATOS FINANCIEROS:
- Yield bruta: {yb:.1f}% | Yield neta: {yn:.1f}% | ROI proyectado 5 años: {roi:.1f}%
- Renta mensual estimada: {renta or 'no indicada'} € | Ocupación objetivo: {ocupacion}%

LUGARES REALES CERCANOS (OpenStreetMap, radio 900m):
{nearby_str}

TIPO DE DOSSIER: {'Para Inversores' if tipo == 'inversores' else 'Para Particulares'}

INSTRUCCIONES CLAVE:
1. Usa tu conocimiento real del mercado de {barrio}, {ciudad} — precios actuales, tendencias, demanda
2. En las propiedades comparables, usa datos REALES de mercado que conozcas de {barrio}/{ciudad}
3. Cada párrafo debe ser ESPECÍFICO y DIFERENCIAL — cero genérico, cero plantilla
4. Tono: broker de lujo con autoridad — directo, preciso, convincente
5. Idioma: {idioma_txt}
6. Payback solo si < 30 años y hay datos de renta

Devuelve ÚNICAMENTE JSON válido sin markdown ni texto adicional:
{{
  "executive_lead": "2-3 frases de impacto que resuman el activo y su singularidad",
  "executive_body": "2-3 frases sobre posicionamiento de mercado y oportunidad",
  "value_proposition": "1 frase concisa de propuesta de valor",
  "buyer_profile": "2-3 frases sobre el perfil del comprador/inversor ideal",
  "investment_thesis_short": "1-2 frases de tesis de inversión",

  "commercial_lead": "2-3 frases de apertura evocadoras y específicas del inmueble",
  "commercial_description": "5-7 frases de descripción profunda: distribución, calidades, espacios, sensación — basada en los datos pero enriquecida",
  "commercial_highlights": "2 frases sobre los aspectos más diferenciales y únicos",

  "location_intro": "2-3 frases de introducción a {barrio} con datos específicos del mercado",
  "location_depth": "4-5 frases de análisis profundo: qué hace único a {barrio}, evolución histórica de precios, perfil de residentes, proyectos urbanos relevantes",
  "location_why_here": "2-3 frases específicas sobre por qué esta ubicación concreta dentro de {barrio} es óptima",

  "services_intro": "2 frases de introducción a los servicios de la zona",
  "services_transport": "2 frases específicas sobre transporte en {barrio} — menciona líneas, tiempos, accesos reales",
  "services_education": "2 frases sobre centros educativos reales de la zona",
  "services_health": "2 frases sobre servicios sanitarios reales de la zona",
  "services_commerce": "2 frases sobre comercio, restauración y ocio real de la zona",
  "services_green": "2 frases sobre parques y espacios naturales de la zona",
  "services_conclusion": "2 frases de síntesis sobre la calidad de vida en la zona",

  "market_context": "3-4 frases sobre el mercado inmobiliario actual en {barrio}/{ciudad}: precios, evolución, oferta, demanda",
  "market_comparable": [
    {{"tipo": "tipo propiedad", "ubicacion": "zona específica en {barrio} o alrededores", "precio": "precio €", "m2": "m²", "nota": "vs nuestro activo"}},
    {{"tipo": "...", "ubicacion": "...", "precio": "...", "m2": "...", "nota": "..."}},
    {{"tipo": "...", "ubicacion": "...", "precio": "...", "m2": "...", "nota": "..."}},
    {{"tipo": "...", "ubicacion": "...", "precio": "...", "m2": "...", "nota": "..."}}
  ],
  "market_positioning": "3-4 frases sobre cómo se posiciona este activo vs la competencia, qué lo hace más atractivo o mejor precio/calidad",
  "market_opportunity": "2-3 frases sobre por qué ahora es el momento y cuál es la ventana de oportunidad",

  "financial_narrative": "3-4 frases de análisis narrativo de rentabilidad — si no hay datos de renta, basa en rendimientos habituales del mercado de {barrio}",
  "investment_thesis": "4-5 frases de tesis completa: demanda, yield, revalorización, perfil, horizonte temporal",
  "investment_demand": "2-3 frases sobre demanda real de alquiler en la zona con datos de mercado",
  "investment_appreciation": "2-3 frases sobre revalorización histórica y proyectada en {barrio}",
  "investment_risks": "2-3 frases honestas sobre riesgos y cómo se mitigan",

  "lifestyle_intro": "2-3 frases sobre el estilo de vida de vivir en {barrio}",
  "lifestyle_body": "3-4 frases sobre restaurants, cultura, ocio, seguridad, comunidad real de {barrio}",

  "advantages": [
    "ventaja específica 1 — diferencial real del inmueble o la zona",
  ...
  "conclusion": "3-4 frases de cierre que sinteticen oportunidad y generen acción — sin ser agresivo",
  "cta": "1 frase de llamada a la acción"
}}"""
    ...
    try:
        ...
        import json as _j
        result = _j.loads(raw)
        print(f'[Claude API] OK — {len(raw)} chars, {len(result)} fields')
        return result
...
def generate_all_content(data):
    ...
    osm_result = _fetch_nearby_places(data)
    if isinstance(osm_result, tuple):
        nearby_places, prop_lat, prop_lon = osm_result
    else:
        nearby_places, prop_lat, prop_lon = [], None, None

    map_b64 = _generate_map_b64(prop_lat, prop_lon, nearby_places) if prop_lat else None

    # ── Claude API: genera todo el contenido analítico ───────────────────────
    ai_content = _gen_with_claude(data, nearby_places, fin, loc_scores, lang)
