...
def gen_commercial_narrative(data, fin, premium_score, lang='es'):
    ...
    def _shorten(text, max_chars=300):
    ...
    if lang == 'es':
        ...
        if descripcion:
            desc_short = _shorten(descripcion.strip().rstrip('.'), 280)
            p3 = f"En cuanto a sus características: {desc_short[0].lower()}{desc_short[1:]}."
        else:
        ...
        if descripcion:
            desc_short = _shorten(descripcion.strip().rstrip('.'), 280)
            p3 = f"As for its features: {desc_short[0].lower()}{desc_short[1:]}."
        else:
